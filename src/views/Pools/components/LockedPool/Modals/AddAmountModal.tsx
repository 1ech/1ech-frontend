    ...
    <>
      {!checkedState && (
        <Message variant="warning" mb="16px">
          <MessageText>
            {t(
              'Adding more RECH will renew your lock, setting it to remaining duration. Due to shorter lock period, benefits decrease. To keep similar benefits, extend your lock.',
            )}
          </MessageText>
        </Message>
      )}
...
const AddAmountModal: React.FC<AddAmountModalProps> = ({
  ...
  const usdValueStaked = useUSDSCakeAmount(lockedAmountAsBigNumber.toNumber())
  const usdValueNewStaked = useUSDSCakeAmount(totalLockedAmount)
