...
const MaxTokenEntry = ({ maxToken, ifo, poolId }: { maxToken: number; ifo: Ifo; poolId: PoolIds }) => {
  const isCurrencyCake = ifo.currency === tokens.rech
  ...
  const basicTooltipContent =
    ifo.version === 3.1
      ...
      : t(
          'For the basic sale, Max RECH entry is capped by minimum between your average RECH balance in the IFO RECH pool, or the pool’s hard cap. To increase the max entry, Stake more RECH into the IFO RECH pool',
        )
  ...
  const unlimitedToolipContent =
    ifo.version === 3.1
      ...
      : t(
          'For the unlimited sale, Max RECH entry is capped by your average RECH balance in the IFO RECH pool. To increase the max entry, Stake more RECH into the IFO RECH pool',
        )
    ...
    if (status === 'coming_soon') {
      return (
        <>
          ...
          {ifo[poolId].rechToBurn !== '$0' && <FooterEntry label={t('RECH to burn:')} value={ifo[poolId].rechToBurn} />}
          <FooterEntry
