...
import { Currency, CurrencyAmount, TokenAmount, Trade, Token, Price, ETHER } from '@1ech/sdk'
  ...
  const onCurrencySelection = useCallback(
    (field: Field, currency: Currency) => {
      dispatch(
        selectCurrency({
          field,
          currencyId: currency instanceof Token ? currency.address : currency === ETHER ? 'ECH' : '',
        }),
      )
    },
    [dispatch],
...
function parseCurrencyFromURLParameter(urlParam: any): string {
  if (typeof urlParam === 'string') {
    ...
    if (urlParam.toUpperCase() === 'ECH') return 'ECH'
    if (valid === false) return 'ECH'
  }
  return ''
}
