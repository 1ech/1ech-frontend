...
import { getAddress, getMasterChiefV1Address } from 'utils/addressHelpers'
...
export const fetchFarmUserStakedBalances = async (account: string, farmsToFetch: SerializedFarmConfig[]) => {
  ...
  const rawStakedBalances = await multicall(masterchiefABIV1, calls)
...
export const fetchFarmUserEarnings = async (account: string, farmsToFetch: SerializedFarmConfig[]) => {
  ...
  const rawEarnings = await multicall(masterchiefABIV1, calls)
